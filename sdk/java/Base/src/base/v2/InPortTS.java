 ...
 * @param <T>
 */
public class InPortTS<T extends Message> extends PortImpl implements IInPort<T> {
    ...
    public InPortTS(CSenseComponent owner, String name) {
	super(owner, name);
    ...
    @Override
    public synchronized IResult onPush(T m) throws CSenseException {
	if (_multiplier == 1) {
	    setMessage(m, true);
	    ...
	    T parent = m;
	    setMessage(m, true);
	    for (int pushIndex = 0; pushIndex < _multiplier; pushIndex++) {		
		_m = (T) views.get(pushIndex);
    ...
    public synchronized void setMessage(Message m, boolean push) throws CSenseException {
	if (_m != null) {
	    m.drop();
	    return; 
	}
    ...
    @Override
    public synchronized T poll() throws CSenseException {	
	if (_pollIndex < 0)  {
	    if (_m != null) throw new CSenseException(CSenseErrors.ERROR);
	    T m = _out.onPoll();
	    if (m != null) {
		setMessage(m, false);
	    }
